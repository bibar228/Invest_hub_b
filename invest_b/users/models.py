...
class User(AbstractBaseUser, PermissionsMixin):
    ...
    email = models.EmailField(max_length=50, unique=False, blank=False)
    login = models.CharField(max_length=50, unique=True, blank=False)
    phoneNumberRegex = RegexValidator(regex=r"^((8|\+7)[\- ]?)?(\(?\d{3}\)?[\- ]?)?[\d\- ]{7,10}$")
    phone = models.CharField(validators=[phoneNumberRegex], max_length=12, unique=False, blank=False)
